...
# ╔═╡ 8ae25b65-f202-4c6c-9409-7550661b1034
using LinearAlgebra
...
ss = getPartialStates(df)

# ╔═╡ 24a91efa-c705-4f53-ab76-89d6e3b61137
length(ss)
...
function Lookahead(state::Int, action::Int, mdp::Mdp, approximationMethod)
	...
	if haskey(t, key)
		for (state, poss) in t[key]
			result += mdp.Y * linearFunction(approximationMethod, state) * poss
		end
	end
	return result
end
...
function Evaluate(state::Int, mdp::Mdp, approximationMethod::Any)
	return maximum([Lookahead(state, action, mdp, approximationMethod) for action in mdp.A])
end
...
function solve(method::NearestNei, mdp::Mdp)
	...
	for state in mdp.S
		...
		chosen_action = -1
		for action in mdp.A
			value = Lookahead(state % 1000, action, mdp, method)
			if value > tmp
				tmp = value
...
function d1(s1, s2)
	pos1 = s1
	pos2 = s2
	return sqrt((pos1 - pos2) * (pos1 - pos2))
end

# ╔═╡ d750628b-653f-494a-9462-29f90cfd7d0d
nn = NearestNei(30, [0.0 for i in 1:length(ss)], ss, d1, 100)
...
# ╔═╡ 174defab-3b04-4d92-bb6f-0115fe7c76c6
ans = Output(p, mdp, filename)

# ╔═╡ 160c3bc0-a24f-4cbd-9f43-961a07f2f91f
histogram(ans)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "61ae7fb7a6f366706e5643934d40d4b17a7b5958"
